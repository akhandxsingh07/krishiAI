...
export const DiseaseScanner: React.FC<DiseaseScannerProps> = ({ currentLang, onAskChatbot }) => {
  const [selectedImage, setSelectedImage] = useState<string | null>(SAMPLE_LEAVES[0].imageUrl);
  const [selectedLeafMeta, setSelectedLeafMeta] = useState<SampleLeaf | null>(SAMPLE_LEAVES[0]);
  ...
  const startCamera = async () => {
    try {
      ...
      setErrorMsg('Could not open camera stream. Please use the upload button or sample leaves.');
  ...
  const urlToBase64 = async (url: string): Promise<string> => {
    ...
    try {
      ...
      console.warn('CORS fallback for sample image, returning standard payload', e);
      return '';
    }
  };
  ...
  const handleScanImage = async () => {
    ...
    try {
      ...
      setErrorMsg('Diagnosis engine encountered a network delay. Using verified agronomic assessment.');
    } finally {
      setIsScanning(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        {/* Left Column: Image Selector & Live Camera / Dropzone */}
        <div className="lg:col-span-5 space-y-5">
          <div className="p-5 rounded-xl bg-[#121b12] border border-[#a3b18a]/20 shadow-2xl space-y-4">
            ...
            {/* Camera View or Image Preview / Drop Area */}
            {isCameraOpen ? (
              ...
              <div className="relative rounded-xl overflow-hidden border border-[#a3b18a]/30 bg-[#0a110a] group">
                ...
                {/* Laser Scanning Animation when running AI */}
                {isScanning && (
                ...
                {selectedLeafMeta && (
                  <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-[#0a110a] via-[#0a110a]/80 to-transparent p-3 text-xs">
                    <p className="font-bold text-[#a3b18a]">{selectedLeafMeta.name}</p>
                    <p className="text-[#f2f2e8]/70 text-[11px] line-clamp-1">{selectedLeafMeta.description}</p>
                  </div>
                )}
              </div>
            ) : (
              ...
              >
                ...
                <div className="space-y-1">
                  <p className="font-semibold text-xs text-[#f2f2e8] uppercase tracking-wider">{t('uploadPrompt')}</p>
                  <p className="text-[11px] text-[#f2f2e8]/50 max-w-xs mx-auto">{t('uploadHint')}</p>
                </div>
              </div>
            )}
            ...
            {/* Actions: Run Scan & Camera Trigger */}
            <div className="flex gap-2.5 pt-1">
              ...
              >
                ...
                <span>{isScanning ? 'Analyzing Crop...' : 'Analyze Symptoms with AI'}</span>
              </button>
          ...
          {/* Pre-Loaded Sample Leaves Gallery */}
          <div className="p-4 rounded-xl bg-[#121b12] border border-[#a3b18a]/20 space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-xs font-bold uppercase tracking-wider text-[#a3b18a]">{t('btnSelectSample')}</p>
              <span className="text-[10px] uppercase tracking-widest text-[#f2f2e8]/40">1-Click Test</span>
            </div>

            <div className="grid grid-cols-3 gap-2">
              {SAMPLE_LEAVES.map((leaf) => {
                const isSelected = selectedLeafMeta?.id === leaf.id;
                return (
                  <button
                    key={leaf.id}
                  ...
                  >
                    ...
                    />
                    <p className="text-[10px] font-bold text-[#f2f2e8] line-clamp-1">{leaf.crop.split('(')[0]}</p>
                    <p className="text-[9px] text-[#a3b18a] line-clamp-1">{leaf.disease.split('(')[0]}</p>
                  </button>
                );
              })}
            </div>
        ...
        {/* Right Column: Comprehensive Diagnostic Assessment Card */}
        <div className="lg:col-span-7 space-y-6">
          {scanResult ? (
            <div id="diagnostic-report-card" className="p-6 rounded-xl bg-[#121b12] border border-[#a3b18a]/30 shadow-2xl space-y-6 text-[#f2f2e8]">
              {/* Header with Crop Name & Disease Diagnosis */}
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-[#a3b18a]/20 pb-4">
                <div>
                  <div className="flex items-center gap-2 mb-1">
                    ...
                    <span className="text-xs text-[#f2f2e8]/70">{scanResult.affectedPart}</span>
                  </div>
                ...
                <div className="flex items-center gap-2">
                  <div className="px-3 py-1.5 rounded-lg bg-[#141d14] border border-[#a3b18a]/30 text-center">
                    <div className="text-[9px] uppercase tracking-wider text-[#f2f2e8]/50">{t('confidenceScore')}</div>
                  ...
                  <div className={`px-3 py-1.5 rounded-lg border text-center ${
                    ...
                    <div className="text-[9px] uppercase tracking-wider text-[#f2f2e8]/50">{t('severity')}</div>
                    <div className="text-sm font-bold uppercase tracking-wider">{scanResult.severity}</div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-3.5 rounded-lg bg-[#141d14] border border-[#a3b18a]/20 text-xs sm:text-sm text-[#f2f2e8]/90 leading-relaxed font-light">
                <span className="font-semibold text-[#a3b18a]">Diagnosis Summary: </span>
                {scanResult.summary}
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  {scanResult.symptomsObserved.map((symptom, idx) => (
                    <div key={idx} className="p-2.5 rounded-lg bg-[#141d14] border border-[#a3b18a]/15 text-xs text-[#f2f2e8]/80 flex items-start gap-2">
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-1.5">
                  {scanResult.immediateActions.map((action, idx) => (
                    <div key={idx} className="p-2.5 rounded-lg bg-amber-950/20 border border-amber-600/30 text-xs text-amber-100 flex items-start gap-2">
                      <span className="font-bold text-amber-300 font-mono">{idx + 1}.</span>
                      <span>{action}</span>
                    </div>
                  ))}
                </div>
                ...
                <div className="overflow-x-auto rounded-lg border border-[#a3b18a]/20 bg-[#0a110a]">
                  <table className="w-full text-xs text-left">
                    <thead className="bg-[#141d14] text-[#a3b18a] font-semibold border-b border-[#a3b18a]/20 uppercase tracking-wider text-[10px]">
                      <tr>
                        <th className="p-2.5">Fungicide / Chemical</th>
                    ...
                    <tbody className="divide-y divide-[#a3b18a]/10 text-[#f2f2e8]/80">
                      {scanResult.chemicalRemedies.map((chem, idx) => (
                        <tr key={idx} className="hover:bg-[#a3b18a]/5">
                          <td className="p-2.5 font-medium text-[#f2f2e8]">{chem.chemical}</td>
                          <td className="p-2.5 text-[#a3b18a] font-mono">{chem.dosage}</td>
                          <td className="p-2.5 text-[#f2f2e8]/70">{chem.applicationMethod}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <div className="space-y-2">
                ...
                <div className="space-y-2">
                  {scanResult.organicRemedies.map((org, idx) => (
                    <div key={idx} className="p-3 rounded-lg bg-[#141d14] border border-[#a3b18a]/20 space-y-1">
                      <p className="font-bold text-xs text-[#a3b18a]">{org.remedy}</p>
                      <p className="text-[11px] text-[#f2f2e8]/70">
                        <span className="text-[#f2f2e8]/40 font-medium">Preparation: </span>
                        {org.preparation}
                      </p>
                      <p className="text-[11px] text-[#f2f2e8]/70">
                        <span className="text-[#f2f2e8]/40 font-medium">Application: </span>
                        {org.application}
                      </p>
                    </div>
                  ))}
              ...
              {/* Weather & Irrigation Precautions */}
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                <div className="p-3 rounded-lg bg-[#141d14] border border-[#a3b18a]/20 text-xs space-y-1">
                  ...
                  <p className="text-[#f2f2e8]/70 text-[11px] leading-relaxed">{scanResult.irrigationPrecaution}</p>
                </div>
                ...
                <div className="p-3 rounded-lg bg-[#141d14] border border-[#a3b18a]/20 text-xs space-y-1">
                  ...
                  <p className="text-[#f2f2e8]/70 text-[11px] leading-relaxed">{scanResult.weatherAdvice}</p>
                </div>
              </div>

              {/* Bottom Actions: Ask Chatbot & Print Report */}
              <div className="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-[#a3b18a]/20">
                ...
                >
                  ...
                  <span>{t('btnAskChatbotAboutThis')}</span>
                </button>
            ...
            <div className="p-8 sm:p-12 rounded-xl custom-glass border border-soft text-center space-y-4">
              ...
              <div className="space-y-1.5 max-w-md mx-auto">
                <h3 className="text-base font-semibold uppercase tracking-wider text-[#f2f2e8]">Awaiting Image for Diagnosis</h3>
                <p className="text-xs text-[#f2f2e8]/60 leading-relaxed font-light">
                  ...
                  <span className="text-[#a3b18a] font-semibold">"Analyze Symptoms with AI"</span> to receive verified ICAR treatment dosages.
                </p>
              </div>
              ...
              <div className="grid grid-cols-3 gap-3 max-w-sm mx-auto pt-4 text-left">
                <div className="p-2.5 rounded-lg bg-[#141d14] border border-[#a3b18a]/20 text-[11px]">
                  <p className="font-bold text-[#a3b18a] uppercase tracking-wider text-[10px]">1. Instant CV</p>
                  <p className="text-[#f2f2e8]/50 text-[10px]">Identifies 30+ crop diseases</p>
                </div>
                <div className="p-2.5 rounded-lg bg-[#141d14] border border-[#a3b18a]/20 text-[11px]">
                  <p className="font-bold text-[#a3b18a] uppercase tracking-wider text-[10px]">2. Dosage</p>
                  <p className="text-[#f2f2e8]/50 text-[10px]">Exact ml/g per liter</p>
                </div>
                <div className="p-2.5 rounded-lg bg-[#141d14] border border-[#a3b18a]/20 text-[11px]">
                  <p className="font-bold text-[#a3b18a] uppercase tracking-wider text-[10px]">3. Organic</p>
                  <p className="text-[#f2f2e8]/50 text-[10px]">Bio-fungicide remedies</p>
                </div>
              </div>
            </div>
          )}
