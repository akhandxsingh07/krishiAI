...
export const X402PaymentCard: React.FC = () => {
  const { activeAccount, signTransactions, wallets, isReady } = useWallet();
  ...
  useEffect(() => {
    ...
    fetch(`${gatewayUrl}/api/x402/status`)
      .then(async (response) => {
        if (!response.ok) {
          throw new Error('Unable to read x402 gateway status.');
        }
      ...
      .catch((err) => {
        if (!cancelled) {
          setError(err instanceof Error ? err.message : 'Unable to read x402 status.');
        }
      });
  ...
  const payAndUnlock = async () => {
    ...
    try {
      ...
      // x402 AVM v2 registration must be done through the official helper.
      // This registers the exact scheme for the Algorand network instead of
      // constructing ExactAvmScheme directly, which can leave the client
      // without a matching network registration.
      ...
      const resourceUrl = `${getX402GatewayUrl()}/api/premium-procurement`;
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Payment request failed (${response.status}): ${body.slice(0, 240)}`,
        );
      }

      const data = await response.json();
      const decodedSettlement = client.getPaymentSettleResponse(
        (name) => response.headers.get(name),
      );

      setSettlement(decodedSettlement || { success: true });
      setUnlockedData(
        data.recommendation || 'Premium procurement intelligence unlocked.',
      );
    } catch (err) {
      ...
      <div className="rounded-2xl border border-[#a3b18a]/30 bg-[#121b12] p-5 sm:p-6 shadow-xl">
        ...
        {!gateway?.enabled && gateway && (
          <div className="mt-4 flex items-start gap-2 rounded-lg border border-amber-500/30 bg-amber-500/10 p-3 text-xs text-amber-200">
            ...
            <span>
              Server wallet is not configured yet. Add <code>AVM_ADDRESS</code> to the server environment before making a real payment.
            </span>
          </div>
        )}
