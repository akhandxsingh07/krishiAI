...
export const IrrigationAdvisor: React.FC<IrrigationAdvisorProps> = ({
  currentLang,
  weatherData,
}) => {
  ...
  const [growthStage, setGrowthStage] =
    useState<GrowthStage>('Vegetative');

  const [soilType, setSoilType] = useState<string>('Alluvial');
  const [acres, setAcres] = useState<number>(2);
  const [irrigationPlan, setIrrigationPlan] =
    useState<IrrigationPlan | null>(null);
  const [isCalculating, setIsCalculating] =
    useState<boolean>(false);

  const [npkCrop, setNpkCrop] =
    useState<string>('Wheat');
  const [npkAcres, setNpkAcres] =
    useState<number>(2);
  ...
  const calculateIrrigation = async () => {
    ...
    try {
      ...
      const data: IrrigationPlan =
        await res.json();

      setIrrigationPlan(data);
    } catch (e) {
      console.warn(
        'Calculating client fallback',
        e,
      );
  ...
  > = {
    Wheat: {
      ...
      zincKg: 10,
      splits:
        'Apply full DAP, MOP & 1/3 Urea at sowing; 1/3 Urea at 1st CRI watering (21 days); 1/3 Urea at tillering.',
    },
    ...
    'Rice / Paddy': {
      ...
      zincKg: 10,
      splits:
        'Apply full DAP, MOP + 1/3rd Urea as basal; 1/3rd Urea at tillering (21 DAT); 1/3rd Urea at panicle initiation.',
    },
    ...
    Cotton: {
      ...
      zincKg: 10,
      splits:
        'Split Urea into 3 equal doses: at thinning, square initiation, and peak flowering.',
    },
    ...
    Sugarcane: {
      ...
      zincKg: 15,
      splits:
        'Apply in 4 split doses: basal planting, 45 DAP, 90 DAP, and final earthing-up (120 DAP).',
    },
    ...
    Tomato: {
      ...
      zincKg: 5,
      splits:
        'Apply half N, full P and K at bed preparation; top dress remaining N in 3 split doses.',
    },
    ...
    Potato: {
      ...
      zincKg: 10,
      splits:
        'Apply 1/2 N + full P & K at planting; remaining 1/2 N during earthing-up (30-35 DAP).',
    },
    ...
    Maize: {
      ...
      zincKg: 10,
      splits:
        '1/3 N + full P & K at sowing; 1/3 N at knee-high stage (V6); 1/3 N at tasseling stage.',
    },
    ...
    Mustard: {
      ...
      zincKg: 8,
      splits:
        '1/2 N + full P, K & Sulphur (20kg/ac) at sowing; remaining 1/2 N at first irrigation (30 DAS).',
    },
  };

  const currentNpk =
    npkGuidelines[npkCrop] ||
    npkGuidelines.Wheat;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
      ...
      <div className="text-center space-y-2">
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#a3b18a]/10 border border-[#a3b18a]/30 text-[#a3b18a] text-xs uppercase tracking-widest font-semibold">
          ...
          <span>
            Smart Agronomy & Water Optimization
          </span>
        </div>
        ...
        <div className="lg:col-span-5 p-5 rounded-xl bg-[#121b12] border border-[#a3b18a]/20 shadow-2xl space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            >
              <option value="Germination">
                Germination / Seedling (अंकुरण)
              </option>

              <option value="Vegetative">
                Vegetative Growth / Tillering (वानस्पतिक वृद्धि)
              </option>

              <option value="Flowering">
                Flowering / Booting (फूल व बाली अवस्था)
              </option>

              <option value="Fruiting / Grain filling">
                Fruiting / Grain Filling (दाना भराव)
              </option>

              <option value="Maturity">
                Maturity / Ripening (पकाव अवस्था)
              </option>
            </select>
          </div>
          ...
          <div className="space-y-1.5">
            ...
            >
              ...
              <option value="Black">
                Black Cotton Soil / Regur (काली मिट्टी)
              </option>

              <option value="Clayey">
                Clayey Heavy Soil (चिकनी मिट्टी)
              </option>

              <option value="Sandy Loam">
                Sandy Loam Light Soil (बलुई दोमट)
              </option>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between text-xs">
              <span className="font-semibold text-[#f2f2e8]/70 uppercase tracking-wider text-[11px]">
                Total Farm Area:
              </span>

              <span className="font-bold text-[#a3b18a] font-mono text-sm">
                {acres} Acre(s)
              </span>
            </div>
            ...
            <div className="p-6 rounded-xl bg-[#121b12] border border-[#a3b18a]/30 shadow-2xl space-y-6 text-[#f2f2e8]">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-[#a3b18a]/20 pb-4">
                ...
                <div className="px-3 py-1.5 rounded-lg bg-[#a3b18a]/20 border border-[#a3b18a]/30 text-[#a3b18a] text-xs font-bold flex items-center gap-1.5 self-start sm:self-auto uppercase tracking-wider text-[11px]">
                  ...
                  <span>
                    Saves ~
                    {irrigationPlan.savingsVsTraditionalPct}%
                    Groundwater
                  </span>
                </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="p-4 rounded-lg bg-[#141d14] border border-[#a3b18a]/30 space-y-1.5">
                  ...
                  <p className="text-2xl font-light font-serif text-[#f2f2e8] font-mono">
                    {irrigationPlan.dripRunTimeHours}{' '}
                    Hours
                  </p>

                  <p className="text-[11px] text-[#f2f2e8]/60 font-light">
                    Run during early morning
                    (6:00 AM - 9:00 AM) to
                ...
                <div className="p-4 rounded-lg bg-[#141d14] border border-[#a3b18a]/15 space-y-1.5">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-[9px] text-[#f2f2e8]/40 uppercase tracking-wider">
                      Conventional
                    </span>
                  ...
                  <p className="text-2xl font-light font-serif text-[#f2f2e8]/60 font-mono">
                    {irrigationPlan.floodIrrigationHours}{' '}
                    Hours
                  </p>

                  <p className="text-[11px] text-[#f2f2e8]/50 font-light">
                    Cycle every{' '}
                    {irrigationPlan.irrigationFrequencyHours}{' '}
                    hours depending on root depth.
                  </p>
            ...
            <div className="flex items-center justify-between">
              ...
              <div>
                <h3 className="text-base font-light font-serif text-[#f2f2e8] flex items-center gap-2">
                  ...
                  <span>
                    Fertilizer (NPK) Calculator per Field Area
                  </span>
                </h3>

                <p className="text-xs text-[#f2f2e8]/60 font-light">
                  Calculated based on ICAR recommended
                  state fertilizer ratios.
                </p>
              </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
              ...
              <div className="p-3 rounded-lg bg-[#141d14] border border-[#a3b18a]/20 text-center space-y-1">
                ...
                <p className="text-lg font-bold text-[#f2f2e8] font-mono">
                  {Math.round(
                    currentNpk.ureaKg * acres,
                  )}{' '}
                  kg
                </p>

                <p className="text-[10px] text-[#f2f2e8]/40">
                  {Math.round(
                    (currentNpk.ureaKg * acres) /
                      45,
                  )}{' '}
