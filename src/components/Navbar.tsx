...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  // Keep the complete navigation visible without squeezing/overlapping.
  // Eureka / Presentation has been removed permanently.
  const navItems = [
  ...
  return (
    <header className="sticky top-0 z-50 w-full bg-[#0a110a]/95 backdrop-blur-md border-b border-[#a3b18a]/20 text-[#f2f2e8] shadow-2xl overflow-x-clip">
      <div className="w-full max-w-[1600px] mx-auto px-3 sm:px-4">
        <div className="flex items-center h-[64px] gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 shrink-0 select-none text-left"
          >
            <div className="w-9 h-9 rounded-full bg-[#a3b18a] flex items-center justify-center text-[#0a110a] font-bold font-serif text-base">
              K
            </div>
            <div className="hidden sm:block leading-none">
              <div className="text-[17px] font-bold tracking-[0.08em] uppercase font-serif">KrishiAI</div>
              <p className="mt-1 text-[8px] text-[#a3b18a] font-medium tracking-wide">Detect • Decide • Act</p>
            </div>
          </button>

          {/* Desktop navigation: fixed compact buttons, no horizontal scrolling */}
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`shrink-0 inline-flex items-center justify-center gap-1.5 px-2.5 py-2 rounded-lg text-[10px] 2xl:text-[11px] font-semibold whitespace-nowrap border transition-colors ${
                    isActive
          ...
          <div className="flex items-center gap-1.5 shrink-0">
            <div className="relative">
              <button
                ...
                className="flex items-center gap-1 px-2 py-2 rounded-lg bg-[#141d14] border border-[#a3b18a]/30 text-[10px] font-semibold whitespace-nowrap"
              >
            ...
            <button
              id="user-auth-btn"
              onClick={onOpenAuth}
              className={`flex items-center gap-1.5 px-2.5 py-2 rounded-lg text-[10px] font-semibold border whitespace-nowrap ${userProfile ? 'bg-[#1a241a] border-[#a3b18a]/40' : 'border-[#a3b18a]/40 hover:bg-[#a3b18a] hover:text-[#0a110a]'}`}
            >
              <User className="w-3 h-3 text-[#a3b18a]" />
              <span>{userProfile ? userProfile.name.split(' ')[0] : 'Login'}</span>
            </button>

            <button id="mobile-nav-toggle" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="lg:hidden p-2 rounded-lg bg-[#141d14] border border-[#a3b18a]/20">
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`flex items-center gap-2 px-3 py-2 rounded-lg text-xs font-medium ${isActive ? 'bg-[#a3b18a]/20 text-[#a3b18a] border border-[#a3b18a]/40' : 'text-[#f2f2e8]/70 bg-[#141d14]'}`}
                >
